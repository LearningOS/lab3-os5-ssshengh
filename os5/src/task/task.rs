...
use crate::task::info::{Info, Priority};
...
impl TaskControlBlock {
    ...
    pub fn pass(&self) -> u64 {
        self.inner_exclusive_access().info.priority.pass
    }

    pub fn record_start(&self) {
        let mut inner_ref = self.inner.exclusive_access();
        inner_ref.info.record_start_time();
    }

    pub fn set_priority(&self, priority: usize) {
        let mut inner_mut = self.inner.exclusive_access();
        inner_mut.info.set_priority(priority);
    }

    pub fn update_pass(&self) {
        let mut inner_mut = self.inner.exclusive_access();
        inner_mut.info.priority.update_pass();
    }
}
